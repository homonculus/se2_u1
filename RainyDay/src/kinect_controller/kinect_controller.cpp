...
//int KinectController::startDevice(){
 ...
 // 	/// [context]
	...
	// /// [registration setup]
	// size_t framecount = 0;
	// std::cout << "KinectController :: END OF START DEVICE " << myListener << " : original : " << &listener << " \n";
	// return 0;
//}
...
KinectImage* KinectController::getDepthImage(){
	...
	std::cout << depth << " ]\n";
	_image->data = depth->data;
	_image->width = depth->width;
	_image->height = depth->height;
...
int KinectController::startDevice(){
	...
	while(!protonect_shutdown && (framemax == (size_t)-1 || framecount < framemax)){
		...
		std::cout << "KINECT CONTRLLER IMAGE RECEIVED!\n";
		waitKey(1000);
		framecount++;
		/// [loop end]
	...
	// Mat m(2, 3, 4);
	// delegate->kinectControllerReceivedImage(m);
